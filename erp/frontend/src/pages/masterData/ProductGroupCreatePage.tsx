...
import { createProductGroup } from '@/api/masterData';
import { isApiError } from '@/api/types';
import { Alert, Button, Card, FormField, Input } from '@/components/ui';

export function ProductGroupCreatePage(): JSX.Element {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    setSubmitting(true);
    try {
      await createProductGroup({ code, name_fa: nameFa, name_en: nameEn });
      navigate('/master-data/product-groups');
    } catch (err) {
  ...
  return (
    <div className="stack">
      <div className="page-header"><h1 className="page-header__title">{t('productGroups.new')}</h1></div>
      <Card>
        <form className="stack" onSubmit={(e) => void handleSubmit(e)} noValidate>
          ...
          <div className="form-actions">
            ...
            <Button type="button" variant="secondary" onClick={() => navigate('/master-data/product-groups')} disabled={submitting}>
              {t('common.cancel')}
            </Button>
          </div>
        </form>
