...
export function AllocationsPage(): JSX.Element {
  ...
  if (!canView) return <></>;
  if (loading) return <Card title={t('shipment.allocations.title', 'Allocations')}><Spinner /></Card>;

  const optLabel = (o: Option): string => o.name_fa || o.code || o.number || o.id;

  return (
    <Card title={t('shipment.allocations.title', 'Allocations')}>
      {error && <Alert variant="danger" title={t('common.error')} onClose={() => setError(null)}>{error}</Alert>}
      {canManage && (
        <form className="stack" onSubmit={(e) => void postAlloc(e)}>
          <h3>{t('shipment.allocations.new', 'New Allocation')}</h3>
          <div className="grid">
            <FormField label={t('shipment.fields.company', 'Company')} required>
            ...
            <FormField label={t('shipment.fields.orderLine', 'Order Line')} required>
            ...
            <FormField label={t('shipment.fields.unit', 'Unit')} required>
            ...
            <FormField label={t('shipment.fields.uom', 'UoM')} required>
            ...
            <FormField label={t('shipment.fields.quantity', 'Quantity')} required>
      ...
      <div className="table-scroll">
        <table className="data-table">
          <thead><tr><th>{t('shipment.fields.unit', 'Unit')}</th><th>{t('shipment.fields.quantity', 'Qty')}</th><th>{t('shipment.fields.status', 'Status')}</th><th></th></tr></thead>
          <tbody>
            {allocations.length === 0 ? <tr><td colSpan={4}>{t('common.noData', 'No data')}</td></tr> : allocations.map((a) => (
              <tr key={a.id}>
                <td>{a.traceability_unit}</td>
                <td>{a.quantity}</td>
                <td>{a.status}</td>
                <td>{a.status === 'RESERVED' && canManage && <Button size="sm" variant="secondary" loading={busy} onClick={() => void doRelease(a.id)}>{t('shipment.actions.release', 'Release')}</Button>}</td>
              </tr>
            ))}
          </tbody>
        </table>
