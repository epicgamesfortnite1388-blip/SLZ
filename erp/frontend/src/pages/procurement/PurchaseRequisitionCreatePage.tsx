...
import { createPurchaseRequisition, type PurchaseRequisition } from '@/api/procurement';
...
function freshLine(): DraftLine {
  return { _key: ++_nextKey, material: '', quantity: '', uom: '', notes: '' };
}
...
export function PurchaseRequisitionCreatePage(): JSX.Element {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    setSubmitting(true);
    try {
      const req: PurchaseRequisition = await createPurchaseRequisition({
        company,
        number,
      ...
      for (let i = 0; i < activeLines.length; i++) {
        ...
        await apiClient.post('/procurement/requisition-lines/', {
          requisition: req.id,
          sequence: i + 1,
          material: l.material,
          quantity: l.quantity,
      ...
      <Card>
        <form className="stack" onSubmit={(e) => void handleSubmit(e)} noValidate>
          ...
          <FormField label={t('procurement.fields.needBy')}>
            {({ id }) => <Input id={id} type="date" value={needByDate} onChange={(e) => setNeedByDate(e.target.value)} disabled={submitting} />}
          </FormField>
          ...
          <div className="form-actions">
            ...
            <Button type="button" variant="secondary" onClick={() => navigate('/procurement/requisitions')} disabled={submitting}>
              {t('common.cancel')}
            </Button>
          </div>
        </form>
