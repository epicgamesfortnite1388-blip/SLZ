"""Sales use-cases (document status transitions + events) — Task 010.

A Sales Order is a transactional instance with a **status state machine** (not
the versioning lifecycle of BOM/Routing/Quality). The only business logic here
is the mechanical, guarded status transition:
...
* guard that only DRAFT orders (and their child lines) may be mutated.

Deliberately NOT here (OPEN business decisions — do-not-build-yet):
* pricing / proforma algorithm (R-14, #11) — ``unit_price`` is manual & nullable;
* ATP / promised-date computation (SR-12, #12);
* allocation / reservation / shipment / invoicing (gated on Q-046 stock layer
  and Finance);
* approval hierarchy / credit gate (#7 / Finance) — ``confirm`` is a single
  manual transition gated by the ``sales.order.manage`` permission at the view.
"""
...
def _assert_declared_status(document: models.Model, to_status: str) -> None:
    ...
    if to_status not in declared:
        raise ConflictError(
            f"'{to_status}' is not a valid status for this order.",
...
def assert_document_editable(document: models.Model) -> None:
    ...
    if not document.is_editable:
        raise ConflictError(
            "This order is not in DRAFT and cannot be modified; use the "
    ...
    *,
    ...
    with atomic_with_events() as events:
        ...
        if locked.status not in set(allowed_from):
            raise ConflictError(
                f"Cannot move this order from '{locked.status}' to '{to_status}'.",
                code="invalid_status_transition",
            )
